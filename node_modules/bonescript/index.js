...
f.getPinMode = function(pin, callback) {
    ...
    var tryReadDirection = function(exists) {
        if (exists) {
            fs.readFile(directionFile, 'utf8', readDirection);
        } else {
            mode.direction = 'unknown';
        }
    };
    var readDirection = function(err, data) {
        if(err) winston.debug('readDirection error: ' + err);
        mode = parse.directionFromFile(data, mode);
    };

    if(callback) {
        my.file_exists(directionFile, tryReadDirection);
